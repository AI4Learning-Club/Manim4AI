...
class CVConfig:
    ...
    bbox_min_iou: float = 0.20          # min IoU to count as overlap
    bbox_min_overlap_ratio: float = 0.35 # min intersection/smaller_bbox ratio
    ...
    ocr_sample_interval: int = 5           # run OCR on every Nth sampled candidate frame
...
@dataclass
class AudioConfig:
    """Parameters for audio feature extraction and AV alignment."""

    # Audio extraction
    sample_rate: int = 16000              # downsample to this rate

    # Silence detection
    silence_threshold_db: float = -40.0   # RMS below this = silent
    silence_min_duration: float = 0.3     # minimum silence interval (sec)
    silence_window_sec: float = 0.1       # analysis window size
    silence_hop_sec: float = 0.05         # analysis hop size

    # Clipping
    clip_threshold: float = 0.99          # amplitude fraction for clipping

    # SNR
    snr_cap_db: float = 60.0             # cap SNR at this value

    # Speech VAD
    speech_energy_threshold_db: float = -35.0
    speech_min_duration: float = 0.2
    speech_merge_gap: float = 0.3

    # Motion-to-segments (for alignment)
    motion_threshold: int = 30
    motion_min_duration: float = 0.2
    motion_merge_gap: float = 0.3
...
class FusionConfig:
    ...
    w_overlap: float = 0.17
    w_rendering: float = 0.17
    w_layout: float = 0.13
    w_animation: float = 0.13
    w_color_consistency: float = 0.08
    w_vlm_semantic: float = 0.17
    w_audio_quality: float = 0.08
    w_av_alignment: float = 0.07
    ...
    pass_vlm: float = 0.60
    pass_audio_quality: float = 0.60
    pass_av_alignment: float = 0.50
...
@dataclass
class ExternalMeta:
    """External metadata injected from agent_pipeline (not computed by eval).

    Maps to paper Table 1 dimensions: Executability, Efficiency, Repairability.
    """
...
class PipelineConfig:
    ...
    meta: ExternalMeta = field(default_factory=ExternalMeta)
    output_dir: Path = Path("eval_output")
    skip_vlm: bool = False                  # run CV-only mode
    skip_audio: bool = False                # skip audio analysis
