...
class LessonBase(AI4LearningBaseScene):
    ...
    def opening_page(self):
        title = self.get_text("框架梳理讲解", font_size=30)
    ...
    def closing_page(self):
        summary = self.get_text("框架梳理讲解的核心不是多讲内容，而是用合适的讲法把理解路径变短。", font_size=22)
        transfer = self.get_secondary_text("迁移问题：如果换一道新题或新概念，你还会沿用“框架梳理讲解”这套理解路径吗？", font_size=18)
        after_class = self.get_secondary_text("课后练习：课后请回忆“框架梳理讲解”最有效的两个讲解动作，并尝试迁移到别的主题。", font_size=18)
